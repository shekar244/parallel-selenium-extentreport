...
# === Configurable Flag ===
REPROCESS_ALL = True  # Set to False if you want to skip already processed files

# === Constants ===
PROCESSED_LOG_FILE = "processed_files.csv"
DATETIME_FORMAT = "%m/%d/%Y %H:%M:%S"

# === Remove the processed file log if reprocessing is enabled ===
def reset_processed_log():
    if os.path.exists(PROCESSED_LOG_FILE):
        os.remove(PROCESSED_LOG_FILE)
        print(f"🗑️  Deleted previous '{PROCESSED_LOG_FILE}' to reprocess all reports.")

# === Load filenames of already processed HTML reports ===
...
# === Extract test info from a single HTML file ===
def extract_tests_from_html(html_path):
    ...
    for test in soup.select("ul.test-collection > li"):
        ...
        # Fallback to last step timestamp if end time missing
...
# === Build summary report grouped by Test Name ===
...
# === Orchestrator to process folder and write Excel ===
...
if __name__ == "__main__":
    folder_to_scan = "Reports"  # Change this to your HTML reports folder
